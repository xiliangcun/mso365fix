...
public class UserTabCtrl {
	...
	public String massCreateUser(@RequestParam(name="prefix") String prefix,
			...
			@RequestParam(name="randomLength", defaultValue="5") String randomLengthStr,
			@RequestParam(name="regexCode", required=false, defaultValue="") String regexCode) {
		...
		int randomLength = 5;
		try { randomLength = Integer.parseInt(randomLengthStr); } catch (Exception e) { return "参数错误：随机位数必须是整数"; }
		if (count < 1 || count > 10000) return "参数错误：数量必须在1到10000之间";
		if (randomLength < 1 || randomLength > 48) return "参数错误：随机位数必须在1到48之间";
		HashMap<String, int[]> map;
		try { map = mcou.createCommonUser(prefix, domain, licenses, userPwd, count, strategy, randomLength, regexCode); }
		catch (IllegalArgumentException e) { return "参数错误：" + e.getMessage(); }
