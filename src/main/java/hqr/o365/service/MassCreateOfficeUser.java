...
public class MassCreateOfficeUser {
	...
	@Autowired
	private GraphUserLicenseService graphLicenseService;
	...
	public HashMap<String, int[]> createCommonUser(String prefix, String domain, String licenses, String userPwd, int count, String strategy, int randomLength, String regexCode){
		String forceInd = "Y";
			...
			if(!"".equals(accessToken)) {
				...
				for(int massCount=0; massCount<count; massCount++) {
					...
					try {
						...
						if(response.getStatusCodeValue()==201) {
							...
							userList.add(graphLicenseService.extractCreatedUserId(response, ou.getUserPrincipalName()));
						}
						else {
							userFail++;
				...
				if(licenses!=null&&!"".equals(licenses)) {
					...
					for(String userId : userList) {
						...
						for (String license : acs) {
							try {
								graphLicenseService.assignLicenseWithRetry(userId, license.trim(), accessToken);
								licenseSucc++;
							} catch (Exception e) {
								System.out.println("许可证分配失败 userId="+userId+" skuId="+license+" error="+e.getMessage());
								licenseFail++;
							}
						}
						...
						if(cgfu.isPresent()) {
							if("Y".equals(cgfu.get().getCd())) {
								...
								createGroupForUser("user-"+userId.substring(0, 8), userId, accessToken);
							}
						}
					}
				}
