				...
				try {
					...
					if(response.getStatusCodeValue()==201) {
						...
						if(licenses!=null&&!licenses.trim().isEmpty()) {
							String createdUserId = graphLicenseService.extractCreatedUserId(response, ou.getUserPrincipalName());
							for (String license : graphLicenseService.normalizeSkuIds(licenses)) {
								try {
									graphLicenseService.assignLicenseWithRetry(createdUserId, license, accessToken);
									message += "成功分配订阅:"+license+"<br>";
								} catch (Exception e) {
									message += "无法分配订阅:"+license+"，"+e.getMessage()+"<br>";
								}
							}
							map.put("message", message);
						}
